...
namespace Crepusculum.NINA.SmartPlugControl.SmartPlugControlDrivers {
    ...
    public class SmartPlugControlWeatherProvider : IEquipmentProvider<IWeatherData> {
        private IProfileService profileService;

        [ImportingConstructor]
        public SmartPlugControlWeatherProvider(IProfileService profileService) {
            this.profileService = profileService;
        }

        public string Name => "SmartPlugControl";

        public IList<IWeatherData> GetEquipment() {
            var devices = new List<IWeatherData>();
            devices.Add(new SmartPlugControlWeatherDriver("MyUniqueId", "MyDeviceId"));
